...
<a href="<?= URL ?>tarifsTransporteur/at" class="btn btn-success d-block">Ajouter</a>
<table class="table text-center">
        ...
        <?php for($i=0; $i < count($tarifs); $i++) : ?>
                <tr>
                        <td class="align-middle border"><?= $nomTransp[$i]['nomTransporteur'] ?>
                        <td class="align-middle border"><?= $tarifs[$i]->getValeurPoids(); ?> kg
                        <td class="align-middle border"><?= $tarifs[$i]->getValeurPrix(); ?> €
                        <td class="align-middle border"><a href="<?= URL ?>tarifsTransporteur/mt/<?= $tarifs[$i]->getIdPrixPoids(); ?>" class="btn btn-warning">Modifier</a></td>
                        <td class="align-middle border">
                        <form method="POST" action="<?= URL ?>tarifsTransporteur/st/<?= $tarifs[$i]->getIdPrixPoids(); ?>" onSubmit="return confirm('Voulez-vous vraiment supprimer le produit ?');">
